...
export default function GeneralLandingPage() {
  ...
  const features = [
    { icon: Camera, text: 'Photo profiles with gallery support' },
    { icon: Heart, text: 'Lifestyle & personality matching' },
    { icon: Sparkles, text: 'AI compatibility recommendations' },
    { icon: Users, text: 'Modern search with advanced filters' },
  ];
    ...
    <div>
      <section className="gradient-hero">
        <div className="container py-16 md:py-24 text-center space-y-6 max-w-3xl mx-auto">
          ...
          <h1 className="text-4xl md:text-5xl font-bold tracking-tight">{c('general')}</h1>
          <p className="text-lg text-muted-foreground">{m('generalDesc')}</p>
      ...
      <section className="container py-16">
        ...
        <ModeSelector selected="GENERAL" />
        <p className="text-center text-sm text-muted-foreground mt-6">{t('trustPrivacy')}</p>
      </section>
    </div>
  );
}
